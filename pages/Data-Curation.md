...
- LESS: Selecting Influential Data for Targeted Instruction Tuning
	...
	- LESS（Low-rank gradiEnt Similarity Search）的算法
		...
		- #### 2.3 任务特定数据选择
		- **验证数据梯度计算**：为目标任务准备验证数据，并计算每个验证子任务的梯度特征。
		- **影响力得分计算**：基于训练数据和验证数据的梯度相似度，计算每个训练数据点的影响力得分。具体公式为：
		  
		  InfAdam(z,z′)=i=1∑N​ηˉ​i​∥∇ℓ(z′;θi​)∥∥Γ(z,θi​)∥⟨∇ℓ(z′;θi​),Γ(z,θi​)⟩​
		  
		  其中，ηˉ​i​是第i个epoch的平均学习率，θi​是第i个epoch后的模型检查点。
		- #### 2.4 选择高影响力数据
		- **选择top-k样本**：根据影响力得分，选择排名前5%的训练样本。这些样本被认为对目标任务最有帮助。
		- #### 2.5 使用选定数据进行训练
		- **目标模型训练**：使用筛选出的高质量数据对目标模型进行训练。目标模型可以使用LoRA或全量微调进行训练。
		- ### 3. 算法特性
		- **与Adam优化器兼容**：LESS将经典影响公式中的梯度特征调整为与Adam优化器和可变长度指令数据兼容。
		- **高效性**：使用LoRA和随机投影构建低维、易于操作的梯度数据存储，实现高效的数据集选择。梯度数据存储可以重复用于新的目标任务。
		- **可转移性**：使用小模型的梯度特征选择的数据，能在大型模型和不同家族的模型中诱导出强大的性能。
		- **可解释性**：定性分析表明，LESS选择的数据在推理和技能类型上与目标任务相似，而现有方法通常基于表面形式线索选择数据。
		- ### 4. 实验结果
		- **MMLU数据集**：使用LESS选择的5%数据进行训练，模型性能接近使用全量数据训练的效果。
		- **TydiQA数据集**：LESS选择的10%数据比随机选择的10%数据提高了4个百分点的准确率。
		- **BBH数据集**：LESS方法也展现出了优越的性能。
		- ### 5. 优势与应用前景
		- **高效性**：LESS通过预热训练和梯度数据存储，显著提高了数据选择的效率。
		- **可转移性**：LESS选择的数据在不同模型和任务中表现出良好的性能，具有很高的通用性。
		- **可解释性**：LESS选择的数据在推理类型上与目标任务更相似，提高了模型的可解释性。
		  
		  通过这些步骤和特性，LESS算法能够有效地从大规模数据集中筛选出对特定任务最有帮助的样本，从而实现更高效的指令调优。
