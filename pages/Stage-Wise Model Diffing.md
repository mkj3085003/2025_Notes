...
- ## Method
	...
	- ### 2. 两种不同的模型和数据集
		- **基础模型（Base Model）**：未经过特殊微调的原始模型。
		- **休眠模型（Sleeper Model）**：已经经过特定数据集微调的模型，表现出休眠代理行为。
		- **基础数据集（Base Dataset）**：用于训练基础模型的普通数据集。
		- **休眠数据集（Sleeper Dataset）**：包含特定触发条件和预设行为的数据集，用于训练休眠模型。
	...
	- ### 4. 微调轨迹
		- **数据优先路径（S→D→F）**：先在基础模型上使用休眠数据进行微调，再在已经微调过的模型上使用休眠数据进行完整微调。
		- **模型优先路径（S→M→F）**：先在休眠模型上使用基础数据进行微调，再在已经微调过的模型上使用休眠数据进行完整微调。
